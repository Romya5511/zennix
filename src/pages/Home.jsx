...
function Home() {
  ...
  async function loadWeeklySpend(hid) {
    ...
    const { data: lastWeekData } = await supabase
      ...
      .select('amount')
      .eq('household_id', hid)
      .gte('bought_at', lastMonday.toISOString())
      .lte('bought_at', lastSunday.toISOString())

    const thisSum = (thisWeekData || []).reduce((sum, e) => sum + (parseFloat(e.amount) || 0), 0)
    const lastSum = (lastWeekData || []).reduce((sum, e) => sum + (parseFloat(e.amount) || 0), 0)
    setWeekTotal(thisSum)
  ...
  function weekChangeText() {
    ...
    if (diff > 0) return { text: `↑ ${pct}% vs last week`, color: 'rgba(255,255,255,0.9)' }
    if (diff < 0) return { text: `↓ ${pct}% vs last week`, color: 'rgba(255,255,255,0.9)' }
    return { text: 'Same as last week', color: 'rgba(255,255,255,0.7)' }
  }
      ...
      <div style={styles.container}>
        ...
        <div style={styles.spendSummaryCard} onClick={() => navigate('/spend')}>
          <div style={styles.spendSummaryLeft}>
            ...
            {change ? (
              <p style={{ ...styles.spendSummaryChange, color: change.color }}>{change.text}</p>
            ) : (
        ...
        <div style={styles.placeholderRow}>
          ...
          <button style={styles.placeholderBtnActive} onClick={() => navigate('/spend')}>
            <span style={styles.placeholderIcon}>📊</span>
